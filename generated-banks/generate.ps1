# 生成 Szy 词库目录结构
$base = "generated-banks\Szy"

function New-WordsJson($dir, $label, $words) {
  $path = "$base\$dir"
  mkdir -Force $path | Out-Null
  $json = @{ label = $label; words = $words } | ConvertTo-Json -Depth 3
  [System.IO.File]::WriteAllText("$path\words.json", $json, [System.Text.UTF8Encoding]::new($false))
}
...
New-WordsJson "$w1\Day1-基础名词" "Day1-基础名词" @(
  ...
  @{ s = "element"; p = "n. 元素（UI 界面上一个按钮是一个界面元素）"; ex = @{ en = "Each UI element has a position."; cn = "每个UI元素都有一个位置。" } }
...
New-WordsJson "$w1\Day2-基础动词" "Day2-基础动词" @(
...
New-WordsJson "$w1\Day3-基础形容词" "Day3-基础形容词" @(
  ...
  @{ s = "physical"; p = "adj. 物理的（角色跳起后会受重力影响）"; ex = @{ en = "The engine simulates physical collisions."; cn = "引擎模拟物理碰撞。" } }
...
New-WordsJson "$w1\Day4-常用副词介词" "Day4-常用副词介词" @(
...
New-WordsJson "$w1\Day5-编程逻辑词" "Day5-编程逻辑词" @(
...
New-WordsJson "$w2\Day6-基础名词二" "Day6-基础名词二" @(
...
New-WordsJson "$w2\Day7-基础动词二" "Day7-基础动词二" @(
...
New-WordsJson "$w2\Day8-基础形容词二" "Day8-基础形容词二" @(
...
New-WordsJson "$w2\Day9-副词介词二" "Day9-副词介词二" @(
...
New-WordsJson "$w2\Day10-编程逻辑词二" "Day10-编程逻辑词二" @(
...
Write-Host "✅ 所有 10 个 Day 的 words.json 已生成"
